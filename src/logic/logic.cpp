...
int updateFiles(){
  ...
  for (const fs::directory_entry &entry : fs::directory_iterator(dirPath)) {
      ...
      if (filename == dbName || !entry.is_regular_file()) continue;
...
WHERE existing_files.name IS NULL;
  ...
  for (int i=0; i<files.size(); i++) {
    sqlite3_stmt* stmt;
    sqlite3_prepare_v2(db, sqlInsertQuery.c_str(), sqlInsertQuery.length(), &stmt, nullptr);
    sqlite3_bind_text(stmt, 1, files[i].c_str(), files[i].length(), SQLITE_STATIC);
    int errCode = sqlite3_step(stmt);
    if (errCode != 0 && errCode != 101) {
        std::cerr << "Insert into db failed, error code: " << errCode << std::endl;
        return 1;
    }
  }
...
char *zErrMsg = 0;
  ...
  rc = sqlite3_exec(db, query_getFiles.c_str(), GetFilesCallback, &files, &zErrMsg);
  if (rc != SQLITE_OK) {
    std::cerr << "SQL error: " + std::string(zErrMsg) << std::endl;
    sqlite3_free(zErrMsg);
